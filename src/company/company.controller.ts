...
export class CompanyController {
    ...
    @Post('create')
    async createCompany(@Body() companyData: CreateCompanyDto, @Req() req: Request & { user: UserDto }) {
        return this.companyService.createCompany(companyData, req.user.userId);
    }

    @Patch('update/:id')
    @UseGuards(AuthGuard('jwt'), ValidateCompanyGuard) 
    async updateCompany(@Body() companyData: CreateCompanyDto, @Param('id') companyId: string, @Req() req: Request & { user: UserDto }) {
        return this.companyService.updateCompany(companyId, companyData, req.user.userId);
    }
    ...
    @Get(':id')
    @UseGuards(AuthGuard('jwt'), ValidateCompanyGuard) 
    async getCompanyById(@Param('id') companyId: string, @Req() req: Request & { user: UserDto }) {

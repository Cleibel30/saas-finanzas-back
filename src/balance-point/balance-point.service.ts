...
export class BalancePointService {
    ...
    async getCompanyBreakEven(companyId: string, startDate: Date, endDate: Date) {
        ...
        const globalMargin = await this.contributionMarginService.getGlobalContributionMargin(companyId, startDate, endDate);
        ...
        const totalFixedCosts = Number(totalFixedCostsAgg._sum.amount) || 0;
        ...
        const breakEvenSalesVolume = globalMargin.globalMarginRatio > 0
            ? (totalFixedCosts / globalMargin.globalMarginRatio)
            : 0;

        const breakEvenSalesVolumeBs = globalMargin.globalMarginRatioBs > 0
        ...
        return {
            ...
            financialsActual: {
                dollars: {
                    ...
                    totalFixedCosts,
                    globalMarginRatio: Number((globalMargin.globalMarginRatio * 100).toFixed(2))
                },
                bs: {
                    totalSalesBs: globalMargin.totalSalesBs,
                    totalVariableCostsBs: globalMargin.totalVariableCostsBs,
                    totalFixedCostsBs,
                    globalMarginRatioBs: Number((globalMargin.globalMarginRatioBs * 100).toFixed(2))
                }
            },
            breakEven: {
                ...
                distanceToBreakEven: Number((globalMargin.totalSales - breakEvenSalesVolume).toFixed(2)),
                distanceToBreakEvenBs: Number((globalMargin.totalSalesBs - breakEvenSalesVolumeBs).toFixed(2))
            }
        };
    }
}
...
export class ContributionMarginService {
    ...
    async getGlobalContributionMargin(companyId: string, startDate: Date, endDate: Date) {
        ...
        const totalInflowAgg = await this.prisma.transaction.aggregate({
            _sum: { amount: true, amountBs: true },
            where: {
                companyId,
        ...
        const totalSales = Number(totalInflowAgg._sum.amount) || 0;
        ...
        const totalVariableCosts = Number(totalVariableCostsAgg._sum.amount) || 0;
        const totalVariableCostsBs = Number(totalVariableCostsAgg._sum.amountBs) || 0;
        ...
        const globalMarginRatioBs = totalSalesBs > 0 ? (totalMarginBs / totalSalesBs) : 0;
    ...
    async getContributionMarginByProductDates(itemId: string, companyId: string, startDate: Date, endDate: Date) {
        // 1. Validar que el producto exista
        const item = await this.prisma.item.findUnique({
            where: { id: itemId, companyId, isRemoved: false },
        });

        if (!item) throw new NotFoundException('El producto especificado no existe.');
        if (item.type === 'SERVICE') throw new BadRequestException('El ítem especificado es un servicio.');
        ...
        const inflowsAgg = await this.prisma.transaction.aggregate({
            _sum: { amount: true, amountBs: true, quantity: true },
            where: {
                itemId: itemId,
                companyId: companyId,
                status: TransactionStatus.COMPLETED,
        ...
        const totalSales = Number(inflowsAgg._sum.amount) || 0;
        const totalSalesBs = Number(inflowsAgg._sum.amountBs) || 0;
        const totalUnitsSold = Number(inflowsAgg._sum.quantity) || 0;

        // 3. OBTENER EL COSTO VARIABLE DESDE LOS LOTES
        // Buscamos todos los lotes de este producto en el rango de fechas para promediar su costo de fabricación
        const batchesInPeriod = await this.prisma.productionBatch.findMany({
            where: {
                itemId: itemId,
                companyId: companyId,
                isRemoved: false,
            ...
            include: {
                // Traemos las transacciones de costo variable de cada lote
                transactions: {
                    where: {
                        status: TransactionStatus.COMPLETED,
                        isRemoved: false,
        ...
        // Calcular el costo variable total invertido en producción durante este periodo
        let totalProductionCost = 0;
        let totalProductionCostBs = 0;
        let totalUnitsProduced = 0;

        for (const batch of batchesInPeriod) {
            totalUnitsProduced += batch.quantity;
            for (const tx of batch.transactions) {
                totalProductionCost += Number(tx.amount) || 0;
                totalProductionCostBs += Number(tx.amountBs) || 0;
            }
        }

        // Calcular el Costo Variable Unitario Promedio Ponderado del periodo
        // Si no hubo producción en este mes, usamos 0 (o podrías buscar el costo del último lote histórico)
        const avgUnitCost = totalUnitsProduced > 0 ? (totalProductionCost / totalUnitsProduced) : 0;
        const avgUnitCostBs = totalUnitsProduced > 0 ? (totalProductionCostBs / totalUnitsProduced) : 0;

        // 4. MATEMÁTICA FINANCIERA AJUSTADA
        // El costo variable real de lo vendido es: Unidades Vendidas × Costo Unitario de Fabricación
        const totalVariableCostOfSales = totalUnitsSold * avgUnitCost;
        const totalVariableCostOfSalesBs = totalUnitsSold * avgUnitCostBs;

        const contributionMargin = totalSales - totalVariableCostOfSales;
        const contributionMarginBs = totalSalesBs - totalVariableCostOfSalesBs;
        ...
        return {
            ...
            productionInPeriod: {
                batchesAnalyzed: batchesInPeriod.length,
                totalUnitsProduced,
                avgUnitCost: Number(avgUnitCost.toFixed(2)),
                avgUnitCostBs: Number(avgUnitCostBs.toFixed(2))
            },
            financials: {
                totalInflow: totalSales,
                totalInflowBs: totalSalesBs,
                totalVariableCost: Number(totalVariableCostOfSales.toFixed(2)), // Costo real de las unidades que se vendieron
                totalVariableCostBs: Number(totalVariableCostOfSalesBs.toFixed(2)),
            ...
            unitAnalysis: {
                ...
                unitContributionMarginBs: Number((unitInflowBs - avgUnitCostBs).toFixed(2)),
            }
        };
    }
    ...
    async getContributionMarginByBatch(batchId: string, companyId: string) {
        ...
        const totalBatchVariableCost = Number(variableCostsAgg._sum.amount) || 0;
        const totalBatchVariableCostBs = Number(variableCostsAgg._sum.amountBs) || 0;

        const totalBatchInflow = Number(inflowsAgg._sum.amount) || 0;
        ...
        const batchContributionMarginBs = totalBatchInflowBs - totalBatchVariableCostBs;

        // El ratio porcentual (%) es el mismo para ambas monedas, usamos los montos en $
        const contributionMarginRatio = totalBatchInflow > 0
            ? (batchContributionMargin / totalBatchInflow)
            : 0;

        // 6. Análisis Unitario ($)
        const unitVariableCost = batch.quantity > 0 ? totalBatchVariableCost / batch.quantity : 0;
        const unitInflow = batch.quantity > 0 ? totalBatchInflow / batch.quantity : 0;
        const unitContributionMargin = unitInflow - unitVariableCost;

        // 7. Análisis Unitario (Bs)
        const unitVariableCostBs = batch.quantity > 0 ? totalBatchVariableCostBs / batch.quantity : 0;
        const unitInflowBs = batch.quantity > 0 ? totalBatchInflowBs / batch.quantity : 0;
    ...
    async getContributionMarginByService(
        ...
        if (item.type === 'PRODUCT') throw new BadRequestException('El ítem especificado no es un servicio.');
        ...
        const totalVariableCost = Number(variableCostsAgg._sum.amount) || 0;
        const totalVariableCostBs = Number(variableCostsAgg._sum.amountBs) || 0;

        const totalInflow = Number(inflowsAgg._sum.amount) || 0;
        ...
        const contributionMarginRatio = totalInflow > 0
            ? (contributionMargin / totalInflow) 
            : 0;

        // 6. Análisis Unitario (Por cada ejecución del servicio)
        const unitVariableCost = totalServicesSold > 0 ? totalVariableCost / totalServicesSold : 0;
        const unitInflow = totalServicesSold > 0 ? totalInflow / totalServicesSold : 0;
        const unitContributionMargin = unitInflow - unitVariableCost;

        const unitVariableCostBs = totalServicesSold > 0 ? totalVariableCostBs / totalServicesSold : 0;
        const unitInflowBs = totalServicesSold > 0 ? totalInflowBs / totalServicesSold : 0;
        const unitContributionMarginBs = unitInflowBs - unitVariableCostBs;
